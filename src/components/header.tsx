...
const Heading: React.FC = () => {
    ...
    <div id="home" className="bg-gray-900 p-4 ">
        <div className=" w-[100%] h-[50px] mx-auto top-0 shadow-lg left-0  items-center  bg-gray-900 fixed flex justify-between">
           ...
           : <IoMdMenu onClick={()=> showdata(!setdata)} className="text-white  text-2xl md:hidden block"/> }</div>
                ...
                <ul className="gap-10  hidden md:flex text-white pr-[115px] pl-[20px]">
                    ...
                    <li className="hover:underline underline-offset-[9px] decoration-4 decoration-blue-500 cursor-pointer  ">Photos</li>
                ...
                <ul className={`w-full h-screen  md:hidden text-white font-bold fixed   bg-black top-[55px]  duration-300 ${setdata ? "left-0" : "left-[-100%]"}`}>
                    ...
                    <li className="p-5 border-b-[1px] border-gray-500"><Link  to="about" offset={50} spy={true} smooth={true} duration={500} onClick={()=>showdata(false)}>About</Link></li>
                    <li className="p-5 border-b-[1px] border-gray-500"><Link to="" onClick={()=>showdata(false)}>Photos</Link></li>
